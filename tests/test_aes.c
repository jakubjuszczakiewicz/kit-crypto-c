...
int test_aes128(void)
{
  ...
  kit_aes_init_128(&lkey, &key2ofs[2]);
  kit_aes_encrypt_block(&lkey, output, &plaintext2ofs[0][2]);
  if (memcmp(output, encrypted_128[0], 16)) {
    fprintf(stderr, "AES 128 encryption fail (1ofs)\n");
    return 1;
  }

  kit_aes_decrypt_block(&lkey, output2, output);
  if (memcmp(output2, plaintext[0], 16)) {
    fprintf(stderr, "AES 128 decryption fail (1ofs)\n");
    return 1;
  }
  ...
  kit_aes_encrypt_block(&lkey, output, &plaintext2ofs[1][2]);
  if (memcmp(output, encrypted_128[1], 16)) {
    fprintf(stderr, "AES 128 encryption fail (2ofs)\n");
    return 1;
  }

  kit_aes_decrypt_block(&lkey, output2, output);
  if (memcmp(output2, plaintext[1], 16)) {
    fprintf(stderr, "AES 128 decryption fail (2ofs)\n");
    return 1;
  }

  kit_aes_encrypt_block(&lkey, output + 2, plaintext[0]);
  if (memcmp(output + 2, encrypted_128[0], 16)) {
    fprintf(stderr, "AES 128 encryption fail (3)\n");
    return 1;
  }
